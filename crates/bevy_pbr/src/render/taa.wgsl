...
fn taa(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    // TODO
    var current_color: vec3<f32> = textureSample(view_target, t_sampler, uv).rgb;
    var previous_color: vec3<f32> = textureSample(taa_accumulation, t_sampler, uv).rgb;
...
fn copy(@location(0) uv: vec2<f32>) -> CopyOutput {
    var out: CopyOutput;
    out.r0 = textureSample(taa_output, t_sampler, uv);
    out.r1 = textureSample(taa_output, t_sampler, uv);
    return out;
}
