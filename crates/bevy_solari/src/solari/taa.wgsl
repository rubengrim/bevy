...
fn disocclusion_checks(history_id: vec2<i32>, history_uv: vec2<f32>, current_position: vec3<f32>, current_normal: vec3<f32>) -> bool {
...
fn taa(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let d_tl = decode_g_buffer_depth(textureLoad(g_buffer, d_id_tl));
    let d_tr = decode_g_buffer_depth(textureLoad(g_buffer, d_id_tr));
    let d_mm = decode_g_buffer_depth(g_buffer_current);
    let d_bl = decode_g_buffer_depth(textureLoad(g_buffer, d_id_bl));
    let d_br = decode_g_buffer_depth(textureLoad(g_buffer, d_id_br));
    ...
    let current_position = depth_to_world_position(d_mm, uv);
    let current_normal = decode_g_buffer_world_normal(g_buffer_current);
    if disocclusion_checks(history_id, history_uv, current_position, current_normal) {
        // Increase accumulated samples by 1, up to 16 total
        accumulated_samples = textureLoad(taa_history, history_id, 0i).a;
    ...
    textureStore(taa_history_output, pixel_id, vec4(current_color, accumulated_samples));
#ifdef TONEMAP
