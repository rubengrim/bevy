...
@group(1) @binding(10)
var<storage, read_write> rays_output: array<RayDesc2>;

@compute @workgroup_size(8, 8, 1)
fn trace_rays(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let pixel_index = global_id.x + global_id.y * u32(view.viewport.z);

    let ray_origin = rays_output[pixel_index].origin.xyz;
    let ray_direction = rays_output[pixel_index].direction.xyz;
